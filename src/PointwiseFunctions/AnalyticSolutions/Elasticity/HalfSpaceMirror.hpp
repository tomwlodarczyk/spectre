 ...
 *
 * \details This solution is mapping thermal noise to an elasticity problem
 * where a normally incident and axisymmetric laser beam with a Gaussian beam
 * profile acts on the face of a semi-infinite mirror. Here we assume the face
 * to be at \f$z = 0\f$ and the material to extend to \f$+\infty\f$ in the
 * z-direction as well as for the mirror diameter to be comparetively large
 * to the `beam width`. The mirror material is characterized by an isotropic
 * homogeneous constitutive relation \f$Y^{ijkl}\f$ (see
 * `Elasticity::ConstitutiveRelations::IsotropicHomogeneous`). In this scenario,
 * the laser introduces a pressure in the form of a Gaussian distribution
 *
 * \f{align}
 * T^{zz} &= \frac{e^{-\frac{r^2}{r_0^2}}}{\pi r_0^2} \\
 * T^{xy} &= T^{yy} = 0 \text{.}
 * \f}
 *
 ...
 * \f}
 *
 * (Eqs. 11 and 12 in \cite Lovelace2007tn), where \f$ \tilde{p}(k)=
 * \frac{1}{2\pi}e^{(\frac{kr}{2})^2}\f$ is the Hankel-Transform of the lasers
 * intensity profile and \f$ \Theta = \mathrm{Tr}(S)\f$ the materials expansion.
 *
 */
 ...
 public:
  ...
  struct IntegrationIntervals {
    ...
    static constexpr OptionString help{
        "Workspace size for numerical integrals"};
